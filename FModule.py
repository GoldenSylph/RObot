...
def show_hist(tmp_main_data):
    fr_data = tmp_main_data['prices'].value_counts()
    fr_data.plot(kind='hist')
    plt.show()

def show_density(tmp_main_data):
    fr_data = tmp_main_data['prices'].value_counts()
    fr_data.plot(kind='density')
    plt.show()

def prepare_kmeans(tmp_main_data):
    pr_content = []
...
def show_c3_hist(tmp_main_data):
    prepare_c3(tmp_main_data)
    tmp_main_data['action'].plot(kind='hist')
    plt.show()

def show_statistics(main_data):
...
def get_dispersion(main_data):
    t_std = main_data['prices'].std()
    return t_std * t_std

if __name__ == '__main__':
    
    main_data = pd.read_csv('full_idex_eth_aura_data1.csv')
    prepare_c3(main_data)
    ...
    k = 286
    gpc = GaussianProcessClassifier(kernel=Matern(), random_state=seed).fit(X_train.sample(k), y_train.sample(k))

    print(rfc.predict_proba([[time.time()]]))
    print(gpc.predict_proba([[time.time()]]))
    
    rfc_results = rfc.predict_proba(X_test)
    gpc_results = gpc.predict_proba(X_test)
    ...
    rfc_data['gpc_rfc'] = False
    
    gpc_data = pd.DataFrame(data=gpc_results, columns=['-1', '0', '1'])
    gpc_data['dates'] = X_test['dates']
    gpc_data['gpc_rfc'] = True
    
    view_data = rfc_data.append(gpc_data)
